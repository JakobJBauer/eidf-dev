...
# Module sets LOG_TO_PVC_ENV and TMUX_LOG_STARTUP for the pod spec.
if [[ -f "${EIDF_DEV_DIR}/modules/tmux-log-to-pvc.sh" ]]; then
  source "${EIDF_DEV_DIR}/modules/tmux-log-to-pvc.sh"
else
  LOG_TO_PVC_ENV=""
  TMUX_LOG_STARTUP=""
fi
